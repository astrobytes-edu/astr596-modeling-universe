...
subtitle: "From Parameters to Functions | Module 6: The Learnable Universe | ASTR 596"
---
...
- Bayesian inference: $p(\boldsymbol{\theta} \\,\vert\\, \mathcal{D})$ → *learning parameters from data*
...
- **Part 1** (This): ML as Bayesian inference in function space
- **Part 2**: Gaussian Processes → exact posterior $p(f \\,\vert\\, \mathcal{D})$ (like Module 5!)
- **Part 3**: Neural Networks → approximate inference (MAP, variational methods)
...
Given data $\mathcal{D} = \{(\mathbf{x}_i, y_i)\}$, we want the posterior over *functions*:
$$
p(f \,|\, \mathcal{D}) \propto p(\mathcal{D} \,|\, f) \cdot p(f)
$$
...
| Prior $p(\boldsymbol{\theta})$ | Prior $p(f)$ (kernel, architecture) |
...
**Module 6**: Learn plausible *functions* from data
...
**Project 5**: Generate N-body simulation data
**Final Project**:
- Use simulations as training data
- Learn $f:$ (initial conditions) $\to$ (cluster evolution)
- Predict outcomes ~100-1000× faster than physics simulation (depending on accuracy requirements)
...
```{admonition} Connection to Module 5: From Parameters to Functions
:class: tip

**Module 5**: Bayesian inference for parameters $\boldsymbol{\theta}$
$$
p(\boldsymbol{\theta} \,|\, \mathcal{D}) = \frac{p(\mathcal{D} \,|\, \boldsymbol{\theta}) p(\boldsymbol{\theta})}{p(\mathcal{D})}
$$

**Machine learning**: Bayesian inference for functions $f$
$$
...
**Two approaches to computing the posterior**:
- **GPs** (Part II): Exact Bayesian inference—full posterior $p(f \,|\, \mathcal{D})$, perfect uncertainty quantification, limited scalability
- **NNs** (Part III): Approximate inference—MAP estimation finds single "best" function, scales to millions of data points
```
...
```{admonition} Connection to Module 5: Model Comparison
:class: note
...
GPs (Part II) perform full Bayesian inference, marginalizing over functions rather than selecting a single "best" function.
```
...
**This is Mean Squared Error when σ² is treated as a known constant!**

**Important caveat**: This equivalence holds *only* if the noise variance σ² is **pre-specified or known**. In Bayesian inference, if σ² is unknown, it becomes a hyperparameter that must be either:
...
In practice, standard neural networks **do not learn σ²**—they implicitly assume a fixed, predetermined noise level. If you want to learn σ² from data, you'd need to either:
1. Add it as an output parameter (heteroskedastic regression)
2. Use a Bayesian approach that marginalizes over it (like GPs)
...
```{admonition} Connection to Module 5: Priors in Action
:class: tip
...
In machine learning, we typically don't compute the full posterior $p(f \,|\, \mathcal{D})$. Instead, we find the **MAP (Maximum A Posteriori)** estimate:
$$
f_{\text{MAP}} = \arg\max_f p(f \,|\, \mathcal{D}) = \arg\max_f \left[ p(\mathcal{D} \,|\, f) \cdot p(f) \right]
$$

Taking the negative log:
$$
f_{\text{MAP}} = \arg\min_f \left[ \underbrace{-\log p(\mathcal{D} \,|\, f)}_{\text{Data loss } \mathcal{L}_{\text{data}}} + \underbrace{-\log p(f)}_{\text{Prior penalty}} \right]
$$
...
$$
p(w_i) = \mathcal{N}(0, \sigma^2) \quad \Rightarrow \quad -\log p(\mathbf{w}) = \frac{1}{2\sigma^2} \sum_i w_i^2 + \text{const}
$$
...
$$
p(w_i) = \frac{1}{2b} \exp\left(-\frac{|w_i|}{b}\right) \quad \Rightarrow \quad -\log p(\mathbf{w}) = \frac{1}{b} \sum_i |w_i| + \text{const}
$$
...
```{figure} #
:name: fig-regularization-priors

**Figure**: L2 vs L1 priors in 2D weight space.
...
$$
\text{Dropout} \approx \text{Sampling from approximate posterior over networks}
$$
...
```{admonition} Regularization = Bayesian Priors = Occam's Razor
:class: important
...
- **L2**: Gaussian prior → smooth functions
- **L1**: Laplace prior → sparse functions (feature selection)
- **Early stopping**: Implicit complexity prior → prevent overtraining
- **Dropout**: Approximate posterior sampling → uncertainty quantification
...
**Gaussian Processes (GPs)**: Build from scratch
