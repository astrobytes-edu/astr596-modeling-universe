...
class StatMechVisualizations:
    ...
    def fig1_temperature_emergence(self):
        ...
        Caption:
        Evolution of velocity distributions as particle number increases from N=1 to N=10,000. 
        Temperature doesn't exist for single particles (top left) but emerges as a well-defined 
        distribution parameter as N increases. The error in temperature measurement decreases 
        as 1/√N, demonstrating how macroscopic properties become precise at large N.
        
        MyST markdown:
        ```{figure} ./figures/fig1_temperature_emergence.svg
        :name: fig-temperature-emergence
        :width: 100%
        :alt: Temperature emergence from ensemble statistics
        ...
        for idx, N in enumerate(N_values):
            ...
            if N == 1:
                ...
                for i, v in enumerate(velocities):
                    color_idx = min(3 + i, len(self.main_colors) - 1)  # Prevent index error
    ...
    def fig2_pressure_from_chaos(self):
        ...
        Caption:
        From molecular chaos to macroscopic pressure. Top: Individual particle momentum transfers 
        vary chaotically. Middle: Distribution of transfers and convergence to steady pressure with 
        increasing N. Bottom: Visual representation of molecules bombarding a wall. Despite random 
        individual impacts, the collective effect produces perfectly steady pressure P = nk_BT.
        
        MyST markdown:
        ```{figure} ./figures/fig2_pressure_chaos.svg
        :name: fig-pressure-chaos
        :width: 100%
        :alt: Pressure emergence from molecular chaos
        ...
        for i in range(n_particles):
            ...
            color_idx = min(2 + i, len(self.main_colors) - 1)
        ...
        for idx, n_samples in enumerate(n_samples_list):
            ...
            averages = []
            for i in range(1, min(n_samples + 1, 1000)):  # Limit iterations for performance
            ...
            color_idx = min(idx, len(self.temp_palette) - 1)
    ...
    def fig3_central_limit_theorem(self):
        ...
        Caption:
        Central Limit Theorem demonstration. Three different starting distributions (uniform, 
        exponential, bimodal) all converge to Gaussian when summed. Top row shows original 
        distributions (N=1), bottom rows show sums of N=2 and N=30 samples. Color coding 
        indicates deviation from Gaussian (green = close, red = far). The KS statistic 
        quantifies convergence. This universal behavior explains why macroscopic physics 
        is predictable despite microscopic randomness.
        
        MyST markdown:
        ```{figure} ./figures/fig3_central_limit.svg
        :name: fig-central-limit
        :width: 100%
        :alt: Central Limit Theorem demonstration
    ...
    def fig4_maxwell_boltzmann_3d(self):
        ...
        Caption:
        Maxwell-Boltzmann distributions at different temperatures. Left: 1D velocity component 
        showing symmetric distribution. Center: 3D speed distribution with v² factor from 
        spherical coordinates. Right: Energy distribution in dimensionless units (E/k_BT) 
        showing the universal form - all temperatures collapse to the same curve when 
        normalized this way. In absolute energy units, higher temperatures would shift 
        the distribution to higher energies. Bottom: 2D velocity space contours showing 
        isotropy. Temperature controls distribution width - higher T means broader 
        distribution (more velocity diversity), not just "faster particles." Dotted lines 
        mark most probable speeds.
        
        MyST markdown:
        ```{figure} ./figures/fig4_maxwell_boltzmann.svg
        :name: fig-maxwell-boltzmann
        :width: 100%
        :alt: Maxwell-Boltzmann distributions at different temperatures
        
        Maxwell-Boltzmann distributions at different temperatures. Left: 1D velocity component 
        showing symmetric distribution. Center: 3D speed distribution with $v^2$ factor from 
        spherical coordinates. Right: Energy distribution. Bottom: 2D velocity space contours 
        showing isotropy. Temperature controls distribution width - higher $T$ means broader 
        distribution (more velocity diversity), not just "faster particles." Dotted lines 
        mark most probable speeds.
        ```
        """
        ...
        for T, color in zip(T_values, self.temp_palette):
            ...
            f_3d = factor * v_cms**2 * np.exp(-self.m_H * v_cms**2 / (2 * self.k_B * T))
        ...
        for T, color in zip(T_values, self.temp_palette):
            ...
            f_E = 2 * np.sqrt(E/np.pi) * np.exp(-E)
    ...
    def fig5_ergodicity_demo(self):
        ...
        Caption:
        Ergodicity demonstration: time averages equal ensemble averages. Top: Coin flip 
        convergence to p=0.5. Middle left: Harmonic oscillator kinetic energy over time 
        converges to E/2. Middle right: Ensemble average from many oscillators at different 
        phases also gives E/2. Bottom: Both methods converge to the same value with 1/√N 
        uncertainty. This equivalence is why MCMC works - exploring parameter space over 
        time is equivalent to sampling the full posterior distribution.
        
        MyST markdown:
        ```{figure} ./figures/fig5_ergodicity.svg
        :name: fig-ergodicity
        :width: 100%
        :alt: Ergodicity demonstration
        
        Ergodicity demonstration: time averages equal ensemble averages. Top: Coin flip 
        convergence to $p=0.5$. Middle left: Harmonic oscillator kinetic energy over time 
        converges to $E/2$. Middle right: Ensemble average from many oscillators at different 
        phases also gives $E/2$. Bottom: Both methods converge to the same value with $1/\sqrt{N}$ 
        uncertainty. This equivalence is why MCMC works - exploring parameter space over 
        time is equivalent to sampling the full posterior distribution.
        ```
        """
    ...
    def fig6_error_scaling(self):
        ...
        Caption:
        Universal 1/√N error scaling in Monte Carlo methods. Left: Log-log plot showing all 
        distributions follow the same error scaling law. Bottom left: Practical implication - 
        achieving 10× better accuracy requires 100× more samples (quadratic cost). Bottom 
        right: Monte Carlo estimation of π showing convergence with 1/√N bounds. This 
        universal scaling sets computational requirements for Projects 3-5.
        
        MyST markdown:
        ```{figure} ./figures/fig6_error_scaling.svg
        :name: fig-error-scaling
        :width: 100%
        :alt: Universal Monte Carlo error scaling
        
        Universal $1/\sqrt{N}$ error scaling in Monte Carlo methods. Left: Log-log plot showing all 
        distributions follow the same error scaling law. Bottom left: Practical implication - 
        achieving $10\times$ better accuracy requires $100\times$ more samples (quadratic cost). Bottom 
        right: Monte Carlo estimation of $\pi$ showing convergence with $1/\sqrt{N}$ bounds. This 
        universal scaling sets computational requirements for Projects 3-5.
        ```
        """
        ...
        for name, (dist_func, color) in distributions.items():
            ...
            for N in N_values:
                ...
                error = np.std(means) if len(means) > 1 else 0
                errors.append(error)
        ...
        theoretical = theoretical * errors[0] / theoretical[0] if errors[0] > 0 else theoretical
    ...
    def fig7_sampling_methods_comparison(self):
        ...
        Caption:
        Comparison of fundamental sampling methods. Top left: Inverse transform method maps 
        uniform [0,1] to any distribution via CDF inversion. Top right: Generated samples 
        perfectly match target. Middle: Rejection sampling "throws darts" at the distribution, 
        accepting points below the PDF curve. Bottom: Method comparison table. These techniques 
        enable all Monte Carlo simulations in Projects 2-5.
        
        MyST markdown:
        ```{figure} ./figures/fig7_sampling_methods.svg
        :name: fig-sampling-methods
        :width: 100%
        :alt: Sampling methods comparison
        ...
        n_samples = 1000
        np.random.seed(42)
        u = np.random.uniform(0.001, 0.999, n_samples)  # Avoid extreme values
        x_samples = stats.norm.ppf(u)
        ...
        efficiency = len(samples) / n_tries
...
if __name__ == "__main__":
    ...
    plt.show()
        ...
        ax3.grid(True, alpha=0.3)
        ax3.set_xlim(1, 1000)  # Set explicit x-limits
