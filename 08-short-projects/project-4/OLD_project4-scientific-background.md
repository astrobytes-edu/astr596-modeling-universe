...
where $D_L^* \equiv D_L(h=1)$ is the luminosity distance with $h$ factored out, and $H_0 = 100h\,\text{km/s/Mpc}$ is the Hubble constant.
...
This is consistent with cosmic microwave background observations (Planck 2018: $|\Omega_K| < 0.001$ where $\Omega_K$ is the curvature parameter).
...
The $\Omega_K a^{-2}$ term in the Friedmann equation doesn't correspond to an energy component with an equation of state. It arises from the geometric contribution to the Friedmann equation in general relativity. When space has positive curvature ($\Omega_K > 0$, closed universe) or negative curvature ($\Omega_K < 0$, open universe), this term captures how geometry affects expansion. For a flat universe, $\Omega_K = 0$ and this term vanishes.
:::
...
$$D_L(z) = \frac{c(1+z)}{H_0}\int_0^z \frac{dz'}{\sqrt{\Omega_m(1+z')^3 + \Omega_\Lambda + (1-\Omega_m-\Omega_\Lambda)(1+z')^2}}$$

where $c = 2.998 \times 10^5\,\text{km/s}$ is the speed of light.
...
- Define the integrand $1/\sqrt{\Omega_m(1+z')^3 + \Omega_\Lambda}$
- Integrate from 0 to $z$
- Multiply by $(c/H_0)(1+z)$ to get $D_L$
- More general (works for non-flat universes too)
...
$$D_L = \frac{c(1+z)}{H_0} I = \frac{2.998 \times 10^5\,\text{km/s} \times 1.5}{70\,\text{km/s/Mpc}} \times 0.5668$$

$$D_L \approx 3639\,\text{Mpc}$$

**Step 3: Compute $D_L^*$ (with $h$ factored out)**

$$D_L^* = h \cdot D_L = 0.7 \times 3639 = 2547\,\text{Mpc}$$

**Step 4: Compute distance modulus**

$$\mu = 25 - 5\log_{10}(h) + 5\log_{10}(D_L^*/\text{Mpc})$$

$$\mu = 25 - 5\log_{10}(0.7) + 5\log_{10}(2547)$$

$$\mu = 25 - 5(-0.155) + 5(3.406) = 25 + 0.775 + 17.03$$

$$\mu \approx 42.81\,\text{mag}$$
...
:::{tip} **Test Your Code**
Use this as a unit test! Your `luminosity_distance()` function should return $D_L^* \approx 2547$ Mpc, and your `distance_modulus()` function should return $\mu \approx 42.81$ mag for these input parameters.
:::
...
$$\ln\mathcal{L}(\theta) = -\frac{1}{2}\sum_{i,j}^n r_i\,[\mathbf{C}^{-1}]_{ij}\,r_j - \frac{1}{2}\ln|\mathbf{C}| - \frac{n}{2}\ln(2\pi)$$
...
**Practical note**: The constant terms $\ln|\mathbf{C}|$ and $\ln(2\pi)$ don't affect MCMC sampling (they cancel in acceptance ratios), so you can omit them:

$$\ln\mathcal{L}(\theta) = -\frac{1}{2}\mathbf{r}^T \mathbf{C}^{-1} \mathbf{r}$$
...
$\Omega_m$ and $h$ are **correlated** (degenerate). You can increase $\Omega_m$ (more matter → slower expansion → objects appear closer) and simultaneously increase $h$ (faster expansion today → objects appear farther) to compensate.
...
Your measurement will fall somewhere between these values (around $h \approx 0.70$), illustrating the tension firsthand. This isn't a textbook exercise — you're exploring an open question at the frontier of cosmology. The resolution of the Hubble Tension may require new physics beyond the Standard Model of cosmology, but that is beyon the scope of this course.
...
- Pen (1999): "A Free-Form Lensing Grid Solution..." ([ApJS 120, 49](https://ui.adsabs.harvard.edu/abs/1999ApJS..120...49P))
...
**The Dark Energy Problem**: Recent DESI results (March 2025) hint that dark energy may not be a true cosmological constant—its equation of state $w$ may **evolve over time** rather than staying fixed at $w = -1$. When DESI combined their baryon acoustic oscillation (BAO) measurements with CMB, supernova, and lensing data, they found 2.8-4.2σ preference for evolving dark energy.
...
- DESI Collaboration (2025): [Year 3 cosmology results](https://data.desi.lbl.gov/doc/papers/)
- [DESI discovers hints of evolving dark energy](https://newscenter.lbl.gov/2025/03/19/new-desi-results-strengthen-hints-that-dark-energy-may-evolve/) (Berkeley Lab press release)
:::
