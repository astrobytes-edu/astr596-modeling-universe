...
const DEFAULTS = {
	...
	projectDescription: 'A project managed by Task Master',
		...
		console.log(
			boxen(
				chalk.cyan.bold('Things you can now do:') +
					...
					chalk.white('1. ') +
					chalk.yellow(
						'Rename .env.example to .env and add your OPENROUTER_API_KEY and PERPLEXITY_API_KEY'
					) +
					'\n' +
					chalk.white('2. ') +
					chalk.yellow(
						'Create a Product Requirements Document (PRD) as a text file (e.g., prd.txt)'
					) +
					'\n' +
					chalk.white('3. ') +
					chalk.yellow(
						'Run `task-master parse-prd <your-prd-file.txt>` to generate initial tasks'
					) +
					'\n' +
					chalk.white('4. ') +
					chalk.yellow('Start managing your project with `task-master` commands!')
			)
		);
	}
}
...
function setupMCPConfiguration(targetDir, projectName) {
	...
	const newMCPServer = {
		'task-master-ai': {
			command: 'npx',
			args: ['-y', 'not-only-claude-task-master-mcp'],
			env: {
