...
- **AI Client Initialization**:
  - ✅ **DO**: Use the utilities from [`mcp-server/src/core/utils/ai-client-utils.js`](mdc:mcp-server/src/core/utils/ai-client-utils.js) (e.g., `getOpenRouterClientForMCP(session, log)`) to get AI client instances. These correctly use the `session` object to resolve API keys.
...
- **AI Interaction**:
  ...
  - ✅ **DO**: Make the AI API call using appropriate helpers (e.g., `_handleAIStream`). Pass the `log` object to these helpers for internal logging. **Do NOT pass `reportProgress`**.
...
- **Environment Variables**: The `session.env` object is critical for AI tools. Pass the `session` object to the `*Direct` function's context, and then to AI client utility functions (like `getOpenRouterClientForMCP`) which will extract API keys and other relevant environment settings (e.g., `MODEL`, `MAX_TOKENS`) from `session.env`.
...
2.  **Create Direct Function File in `mcp-server/src/core/direct-functions/`**:
    ...
    - Implement `async function yourCommandDirect(args, log, context = {})` using **camelCase** with `Direct` suffix. **Remember `context` should only contain `{ session }` if needed (for AI keys/config).**
        ...
        - **Handle AI (if applicable)**: Initialize clients using `getOpenRouterClientForMCP(session, log)`, build prompts, call AI, parse response. Handle AI-specific errors.
