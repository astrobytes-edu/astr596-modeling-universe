...
# OLD Chapter 1: Computational Environments & Scientific Workflows
...
- [ ] You have completed the {doc}`../getting-started/index.md` module (`basic setup`, `CLI`, `git`).
...
:::{important} 💡 Computational Thinking: Reproducible by Design
...
Copy and paste this `Python` script into a text editor and save as save as mass_ratio.py. Run it from the terminal with: `python mass_ratio.py` or directly in `IPython` with: `run mass_ratio.py`.
